...
import (
	...
	traceconfig "webapp-jaeger/tracelib"
...
func main() {
	...
	tracer, closer := traceconfig.TraceInit("web-tracer-client", "const", 1)
	defer closer.Close()
	...
	sn := randomString(16) //nolint:gomnd
	...
	if err != nil {
		fmt.Println("inject err:", err.Error())
		return
	}
...
func saveResponse(response []byte) error {
	...
	if err != nil {
		fmt.Println("write response to file error : ", err)
		return err
	}
...
func sendRequest(ctx context.Context, req *http.Request) {
	...
	go func(req *http.Request) {
		...
		saveResponse(body) //nolint:errcheck
